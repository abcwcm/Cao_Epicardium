...
* load data onto Box --> add functions for loading the data; regenerate the plots
* focus on samples we've actually worked with
...
data_dirs <- paste0("/scratchLocal/frd2007/2019_06_Jingli/data/CellRanger/", samples, "/outs/filtered_feature_bc_matrix/")
...
##! save(sce, file = "/scratchLocal/frd2007/2019_06_Jingli/data/sce_original.rda")
...
source("/scratchLocal/frd2007/2019_06_Jingli/src/qc_plots.R")
...
**We therefore focused on the *tcf21*-sorted samples from now on.**
...
##!save(gns_dropouts, file = "/scratchLocal/frd2007/2019_06_Jingli/data/gns_dropouts.rda")
...
## Excluding Tcf21-low clusters

```{r eval=TRUE, cache.lazy=FALSE}
scf <- readRDS("sce_CellGeneFiltWithScTransformBatchCorrect_2019-06.rds")

gns <- as.data.frame(rowData(scf)[, 1:2])
row.names(gns) <- rownames(scf)
gns <-  as.data.table(gns, keep.rownames = TRUE)
```
```{r cache=FALSE}
cluster_cols <- ABCutilities:::fx.get_palette_ABC("paired_pal")[1:length(unique(scf$res.03))]
names(cluster_cols) <- levels(scf$res.03)

condition_cols <- c("cyan4","blue2","gray85")
names(condition_cols) <- as.character(unique(scf$condition))
```

```{r summary_allClusters, fig.width = 15, fig.height = 12}
p1 <- scABC2::plot_reducedDim_from_sce(scf, which_reddim = "umap_scTransformBatch", which_pcs = c(1:2),
...
p2 <- scABC2::plot_reducedDim_from_sce(scf, which_reddim = "umap_scTransformBatch", which_pcs = c(1:2),
                         exprs_values = "log1p_sctransform.batch",
                         color_by = "tcf21", 
                         label_by = "res.03",
                         alpha = .3, set_colors = TRUE,
...
p3 <- make_long_dt(scf, genes = "tcf21", exprs_values = "log1p_sctransform.batch", include_metaData = c("condition","res.03")) %>% ggplot(., aes(x = res.03, y = log1p_sctransform.batch, fill = res.03)) + 
    geom_violin(draw_quantiles = TRUE) +
    ggbeeswarm::geom_quasirandom(size = 1, shape = 21, alpha = .7) + 
    facet_grid(~condition) + 
    xlab("cluster") + ylab("tcf21 expression") +
...
```{r dot_plot_markers, fig.width = 10, message=FALSE, warning = FALSE}
mks_mn <- scABC2::prep_dot_plot(scf, genes = c("tcf21",
                                               "tpm1", "myl7", # cardiomyocytes
                                               "mpeg1", # macrophages
                                               "fli1a"), # endothelial cells
                    exprs_values = "log1p_sctransform.batch", 
                    group_by = "res.03")
setorder(mks_mn, res.03)

ggplot(mks_mn, aes(x = factor(gene, levels = c("tcf21","tpm1", "myl7", "mpeg1", "fli1a"), ordered = TRUE),
                   y = res.03, color = mean.norm.exprs, size = pct.expressed)) +
  geom_point() +
  scale_color_gradient(low = "grey85", high = "red3") +
  ylab("cell cluster") + xlab("genes (Epicardial cells | CM/SMC | M0 | Endothelial)")+
  theme(legend.position = "bottom") +
  ggtitle("Selected marker gene expression patterns across the cells of different clusters")
```

* Based on the expression of these and other marker genes,  we assume that:
    - cluster 6 = smooth muscle cells
    - cluster 10 = cardiomyocytes
    - cluster 8 = macrophages
    - cluster 11 = endothelial cells
    
To focus on cells of the epicardium, we're going to exclude these clusters.

## Reprocessing data after cluster removal

```{r removing_clusters_and_reprocess, eval=FALSE}
scf <- scf[, !(as.character(scf$res.03) %in% c("6","8","10","11"))]
scf <- calculateQCMetrics(scf)

## remove genes with fewer more than 99.85% dropouts (approx. 5 cells per condition must
## express a given gene
gns_drps <-  scABC2::calc_dropouts_per_cellGroup(scf, rownames(scf), split_by = "Sample")
keep_gns <- unique(gns_drps[pct.zeroCov_cells <= 99.85]$gene)
scf <- scf[keep_gns,]

## re-do clustering ===========================================
srt <- CreateSeuratObject(counts = counts(scf), 
                          project = "Epi_clusterRemove", assay = "RNA",
                          min.cells = 0, min.features = 0,
                          names.field = 1, 
                          meta.data = as.data.frame(colData(scf)[, c("Sample","condition")])
                          )

srt <- SCTransform(srt, new.assay.name = "scTrans_batch_ClustRemove", batch_var = 'condition')

srt <- RunPCA(object = srt, features = VariableFeatures(object = srt), do.print = FALSE)
png("elbow_plot_scTransformBatchClustRemove.png")
ElbowPlot(object = srt) # -->  to determine the # of dims for tSNE
dev.off()

png("pca_dimLoadings_scTransformBatchClustRemove.png", height = 1900, width = 1500)
VizDimLoadings(srt, dims = 1:20, reduction = "pca")
dev.off()

png("pca_dimPlot_scTransformBatchClustRemove.png", height = 1500, width = 1500)
DimPlot(srt, reduction = "pca",  group.by = "orig.ident")
dev.off()

png("pca_dimHeatmaps_scTransformBatchClustRemove.png", height = 1500, width = 800)
...
saveRDS(srt, file = "seurat_scTransformBatchCorrectedClustersRemoved.rds")
```

```{r regenerating_scf_object, eval=FALSE}
cd_srt <-  srt@meta.data
scf$clusters.03 <- cd_srt[colnames(scf),]$scTrans_batch_ClustRemove_snn_res.0.3
scf$clusters.08 <- cd_srt[colnames(scf),]$scTrans_batch_ClustRemove_snn_res.0.8

### reduced dimensions
reducedDim(scf, "pca_scTransformBatch") <- Embeddings(object = srt, reduction = "pca")[colnames(scf),]
reducedDim(scf, "umap_scTransformBatch") <- Embeddings(object = srt, reduction = "umap")[colnames(scf),]
...
assay(scf, "log1p_sctransform.batch") <- normd.m

saveRDS(scf, file = "sce_CellGeneFiltWithScTransformBatchCorrect_ClustRemoved_2019-07.rds")
```
...
BLAS/LAPACK: /pbtech_mounts/homes022/frd2007/miniconda3/envs/muR/lib/R/lib/libRblas.so
